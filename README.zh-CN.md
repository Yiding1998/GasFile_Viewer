...
- [直接打开 Garfield 气体文件工作台](https://yiding1998.github.io/GasFile_Viewer/garfield_gas_workbench_pro.html)：载入、比较、绘图、分析并导出一个或多个 Garfield 气体文件的数据。
...
气体文件工作台是一个单文件网页应用。将一个或多个 **.gas** 文件拖入页面，或者通过文件选择器载入，即可查看和比较文件数据。在线版本无需安装软件，也无需启动本地网页服务器。

如需离线使用，可以下载 [garfield_gas_workbench_pro.html](garfield_gas_workbench_pro.html)，然后直接使用现代浏览器打开。
