...
如需离线使用，请将 [garfield_gas_workbench_pro.html](garfield_gas_workbench_pro.html) 和 [gas-file-parser.js](gas-file-parser.js) 放在同一目录，再使用现代浏览器打开 HTML。本地文件载入、绘图、分析和项目操作仍然可用；仓库检索需要通过 GitHub Pages 或能够提供完整仓库文件的本地网页服务器打开。
