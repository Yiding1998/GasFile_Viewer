...
- **原生前端**：单文件工作台容易分发；代价是两个本地化工作台存在大量平行代码，需要严格同步。
...
该文件包含页面结构、响应式 CSS 和页面控制代码，加载 `gas-search-core.js` 作为唯一共享依赖。
...
两者都是自包含应用：HTML 定义全部工作台控件，CSS 定义布局，内联 JavaScript 完成解析、状态、绘图、分析与导出；末尾再加载共享检索核心和仓库集成库。
...
关键函数：`parseGasFile`、`parseLevels`、`nums`、`section`。
...
数值支持 Fortran `D` 指数。若网格数量不完整或数值少于预期，解析立即报错。多余数值记录在完整性信息中，不会悄悄改变维度。
...
`workbench-library.js` 传入已验证的文本和来源元数据。`addGasTexts` 用来源哈希或路径去重，逐个调用 `parseGasFile`，单个失败不会阻止其余文件加入。
...
`garfield_gas_multi_file_viewer_advanced_legend.html` 是独立中文单文件应用，拥有自己的解析器、绘图、交互、图例和导出代码。它不加载 `gas-search-core.js` 或 `workbench-library.js`，也不参与当前工作台项目格式。
...
4. 重新生成 pretty 索引。
5. 用 `--check` 验证生成结果。
6. 非 pull request 时上传整个仓库为 Pages artifact。
...
4. 更新 `gas-search-core.js` 或页面消费者。
5. 更新双语文档和索引报告。
...
当前工作台本体不是运行时语言包架构。新增语言意味着复制并同步完整 `pro` 页面；仓库检索弹窗可在 `workbench-library.js` 增加语言文案。长期维护第三种语言前，建议先把工作台逻辑拆成共享 JS，再让 HTML 只保留本地化结构和文本。
...
1. 将两个 Pro 工作台的共同 JavaScript 提取为共享模块，只保留本地化文案差异。
2. 为 `GasSearchCore.evaluate` 增加可在 Node 或浏览器运行的数值单元测试。
3. 把 Garfield 解析器提取为共享模块，并用最小真实样例覆盖二维/非二维、误差、激发和电离记录。
4. 增加浏览器端到端测试，覆盖加载、检索加入、绘图和项目往返。
5. 对索引 schema 和项目 JSON 提供机器可读 JSON Schema，便于兼容性检查。

这些重构应分步进行，并先用现有行为测试固定输出，避免在模块拆分时同时改变物理换算或科研结果。
